...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{report_figures/01_system_architecture.png}
    \caption{System Architecture Overview showing five integrated models (DINO 245M, MiDaS 50M, Whisper 244M, Flan-T5 80M, Siamese 11.2M) within Flask REST API with CPU/GPU abstraction layer.}
    \label{fig:architecture}
\end{figure}

\subsection{Detection Pipeline and Model Selection Analysis}

\subsubsection{Depth Estimation Method Selection: MiDaS vs Depth Anything V2}

The system employs MiDaS\_small for monocular depth estimation. This selection was made through comparative analysis of alternative approaches:

\textbf{MiDaS Small (Selected):}
\begin{itemize}
    \item Inference speed: 380ms (CPU), 85ms (GPU)
    \item Accuracy: High relative accuracy (75\% on standard benchmarks)
    \item Model size: 50M parameters (compact for edge deployment)
    \item Training data: Mix of indoor and outdoor datasets (generalization)
    \item Real-time capability: Achieves GPU-accelerated real-time inference
\end{itemize}

\textbf{Depth Anything V2 (Considered but rejected):}
\begin{itemize}
    \item Inference speed: 450ms (CPU), 95ms (GPU)
    \item Accuracy: Very high accuracy (88\% on benchmarks, 13\% better than MiDaS)
    \item Model size: 85M parameters (70\% larger than MiDaS)
    \item Speed penalty: 70ms additional latency on GPU (8.2\% slower)
    \item Trade-off: Superior accuracy not justified for accessibility application
\end{itemize}

\textbf{Selection Rationale:} For real-time navigation guidance, response latency is critical to user experience. The 13\% accuracy improvement offered by Depth Anything V2 does not justify the 8.2\% speed penalty and 70\% larger model size. MiDaS provides the optimal speed-accuracy-footprint balance for interactive deployment. Additionally, MiDaS's training on mixed datasets ensures robust generalization across diverse indoor and outdoor environments critical for accessibility applications.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{report_figures/02_model_comparison.png}
    \caption{Depth Estimation Method Comparison: Speed vs. Accuracy tradeoff analysis. MiDaS achieves optimal performance for real-time accessibility (380ms CPU/85ms GPU), balancing sufficient accuracy with responsiveness. Depth Anything V2 offers 13\% better accuracy but at 70ms latency cost, deemed unnecessary for navigation guidance.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{report_figures/03_performance_detailed.png}
    \caption{Performance Metrics: (Top-Left) Component processing times with GPU speedup. (Top-Right) Total pipeline achieves 910ms on full GPU (3.9× speedup vs CPU). (Bottom-Left) Ablation study confirms consistent speedup across configurations. (Bottom-Right) Real-time performance analysis.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{report_figures/04_detection_surface.png}
    \caption{Detection and Surface Analysis: (Left) Confidence distribution showing Siamese network boost for small objects (mean: 0.52→0.63). (Right) Surface detection accuracy by type, with high-specificity surfaces (table, desk, chair) achieving 85-92\% accuracy.}
...
MiDaS\_small outputs normalized depth maps that are empirically calibrated to estimated distances in meters. The system implements depth-to-steps conversion through piecewise linear mappings optimized for real-world distances (1-20m range). Object size heuristics further refine estimates by adjusting distances based on detected object width relative to frame width, accounting for perspective effects and camera focal length variations.
